<template>
  <div class="shopManage">
    ...
    <div class="box">
      <div class="btns">
        <el-button @click="add" type='primary' size='small' class="add"><i class="el-icon-plus"></i>  新增店铺</el-button>
    ...
    <el-dialog
      ...
      <el-form :label-position="labelPosition" label-width="80px" :model="shopForm" :rules='rules' ref='shopForm' class="shopForm">
        ...
        <el-form-item label="联系人姓名" prop='storePerson' >
          <el-input v-model="shopForm.storePerson" maxlength='15'></el-input>
        </el-form-item>
...
export default {
  ...
  methods:{
    ...
    getShopList(){
      this.$http({
        method:'get',
        url:''+process.env.API_ROOT+'/shop/list?shopName='+(( this.select == '店铺名称') ?  this.selStr :'') +'&shopCode='+((this.select == '店铺ID') ?  this.selStr :'')+'&unlimited='+((this.select == '不限') ?  this.selStr :'')  +'&creatTime='+(this.time ===null   ? '' : this.time.length!=0 ? this.time[0] :'')  +'&endTime='+(this.time ===null   ? '' :this.time.length!=0  ?  this.time[1] :'')  +'&pageIndex='+this.currentPage +'&pageSize=' + this.pageSize,
      }).then(res => {
