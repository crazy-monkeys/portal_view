<template>
  <div class="account">
    ...
    <div class="box">
      ...
      <div class="tab">
        <el-table :data="tableData" style="width: 100%" height="700" @row-click='rowClick'>
          ...
          <el-table-column prop="loginName" label="账户" show-overflow-tooltip>
          </el-table-column>
          <el-table-column prop="realName" show-overflow-tooltip label="姓名">
          </el-table-column>

          <el-table-column prop="phone" show-overflow-tooltip label="手机号码">
          </el-table-column>
          ...
          <el-table-column prop="" show-overflow-tooltip label="最后登录时间">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" width="100" fixed='right'>
            <template slot-scope="scope">
              <el-button @click.native.prevent="changeStatus(scope.$index, tableData)" type="text" :disabled="tableData[scope.$index].status==2"
                :class="tableData[scope.$index].status==2?'dis':''">
                {{tableData[scope.$index].status!=3 &&tableData[scope.$index].status!=2? "禁用" :"启用"}}
              </el-button>

              <el-button @click.native.prevent="mod(scope.$index, tableData)" type="text" :disabled="tableData[scope.$index].status==2"
                :class="tableData[scope.$index].status==2?'dis':''">
                修改
              </el-button>
            </template>
    ...
    <!-- 添加店铺控件 -->
    <el-dialog title="修改" top='8vh' :visible.sync="dialogVisible" width="600px" :before-close="close"
          ...
          <el-tab-pane label="修改用户组" name="4">
            <div class="groupBox">
              <el-form-item label="" prop='groups'>
                <el-tree @check-change='a' :data="groups" show-checkbox node-key="userGroupId" :props="defaultProps"
                  :filter-node-method="filterNode" :default-checked-keys="form.groups" ref="tree">
                </el-tree>
              </el-form-item>
            </div>
          </el-tab-pane>
