...
updateTime: 1555060284000 -->
      <div class="tab">
        <el-table :data="tableData" style="width: 100%" height="700" @row-click='rowClick'>
          ...
          <el-table-column prop="noticeModule" show-overflow-tooltip label="模块key">
          </el-table-column>
          <el-table-column prop="type" label="关联类型" show-overflow-tooltip>
          ...
          <el-table-column prop="noticeValue" show-overflow-tooltip label="关联值">
          </el-table-column>
          ...
          <el-table-column prop="active" width='80' show-overflow-tooltip label="状态">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" width="150" fixed='right'>
            <template slot-scope="scope">
              <el-button @click.native.prevent="changeStatus(scope.$index, tableData)" type="text">
                {{tableData[scope.$index].active==1 ? "禁用" :"启用"}}
              </el-button>
              <el-button @click.native.prevent="mod(scope.$index, tableData)" type="text">
                配置服务
              </el-button>
        ...
        <div class="block">
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage"
            :page-sizes="[10,20,40]" :page-size="10" layout="sizes,total, jumper, prev, pager, next" :total="total">
          </el-pagination>
        </div>
      </div>
    </div>
    ...
    <!-- 修改 -->
    <el-dialog :title="edit ?'配置服务': '新增配置'" top='8vh' :visible.sync="dialogVisible" width="600px" :before-close="close"
      ...
      <el-form label-position="top" label-width="80px" :model="form" :rules='rules' ref='form' class="form">
        <el-form-item prop='phone' label='短信通知服务' v-if="edit">
          <el-select v-model="form.phone" placeholder="请选择">
            <el-option v-for="item in smsList" :key="item.noticeRegisterId" :label="item.signature" :value="item.noticeRegisterId">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop='email' label='邮件通知服务' v-if="edit">
          <el-select v-model="form.email" placeholder="请选择">
            <el-option v-for="item in edmList" :key="item.noticeRegisterId" :label="item.signature" :value="item.noticeRegisterId">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item prop='value' label='关联值' v-if="!edit">
          <el-select v-model="form.value" placeholder="请选择" filterable>
            <el-option v-for="item in  form.type=='MERCHANT' ? merchantList :shopList" :key="form.type=='MERCHANT' ? item.apiMerchantId :item.shop_info_id "
              :label="form.type=='MERCHANT' ? item.apiMerchantName :item.shop_name " :value="form.type=='MERCHANT' ? item.apiMerchantId :item.shop_info_id ">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
