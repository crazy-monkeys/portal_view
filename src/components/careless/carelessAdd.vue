    ...
    <div class="addAct" >    
       <div class="head clear">   
         <el-breadcrumb separator="/">
          ...
          <h1 v-if="$route.params.type==8"> <i class="el-icon-back" @click="back(0)"></i> <span >新建预付款提醒</span> </h1>  
          <h1 v-if="$route.params.type==10"> <i class="el-icon-back" @click="back(0)"></i><span >新建退货提醒</span></h1>  
          <h1 v-if="$route.params.type==7"> <i class="el-icon-back" @click="back(0)"></i><span >新建商品回购提醒</span></h1>  
          <h1 v-if="$route.params.type==6"> <i class="el-icon-back" @click="back(0)"></i><span >新建好评关怀</span></h1>  
          <h1 v-if="$route.params.type==2"> <i class="el-icon-back" @click="back(0)"></i><span >新建付款关怀</span></h1>  
          <h1 v-if="$route.params.type==3"> <i class="el-icon-back" @click="back(0)"></i><span >新建发货提醒</span></h1>  
          <h1 v-if="$route.params.type==1"> <i class="el-icon-back" @click="back(0)"></i><span >新建订单催付</span></h1> 
            ...
            <el-form-item label="短信内容" prop="textarea" >
              <div style="float:left;position:relative">
                ...
                <span class="shop">
                 ...
                 <span class="td" v-if="$route.params.type == 8 || $route.params.type == 7 ||$route.params.type == 1">
                    退订回TD 
                  </span>
    ...
    num:{
      get: function () {
        if(this.$route.params.type==1 ||this.$route.params.type==7 ||this.$route.params.type==8 ){
          return this.sign.length+7+this.ruleForm.textarea.length
        }else{
          return this.sign.length+this.ruleForm.textarea.length+2
        }
      },
      set: function () {
      }
    ...
    "ruleForm.textarea": {
      handler: function(n, o) {
        ...
        if(n!=o){
          //计算字数
          // if(this.$route.params.type==1 ||this.$route.params.type==7 ||this.$route.params.type==8 ){
          // return this.$store.state.shopId.shopName.length+7+this.ruleForm.textarea.length
      ...
      this.$http({
        ...
        url:''+process.env.API_ROOT+'/marketing/sms/signatures?' + 'shopInfoId=' + shopInfoId,
          ...
          if(this.signList.length==0){
            ...
            this.ruleForm.sign = this.signList[0].noticeRegisterId;
            this.sign = this.signList[0].channelDesc
          }
        })
        .catch(error => {
          console.log(error);
