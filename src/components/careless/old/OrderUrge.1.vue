    ...
    <div v-if='none'>
      ...
      <div class="box">
        <div class="btns clear">
          <!-- 新建按钮 -->
          <el-button @click="add" class="add" v-if='!ruleForm.marketingPlanId'>新建订单催付</el-button>
          <el-button @click="add" class="add" v-if='ruleForm.marketingPlanId'>编辑订单催付</el-button>
        </div>
        <div class="tab">
          <el-table
            :data="tableData"
    ...
    <div class="addAct" v-if="!none && !rep && !look">    
       <div class="head clear">   
         <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item>客户关怀</el-breadcrumb-item>
          <el-breadcrumb-item>订单催付</el-breadcrumb-item>  
          <el-breadcrumb-item v-if='!ruleForm.marketingPlanId'>新建订单催付</el-breadcrumb-item>      
          <el-breadcrumb-item v-if='ruleForm.marketingPlanId'>编辑订单催付</el-breadcrumb-item>      
        </el-breadcrumb>
        <h1 v-if='!ruleForm.marketingPlanId'> <i class="el-icon-back" @click="back(0)"></i>  <span >新建订单催付</span></h1>
        <h1 v-if='ruleForm.marketingPlanId'> <i class="el-icon-back" @click="back(1)"></i>  <span >编辑订单催付</span></h1>
      ...
      <div class="form">
        <el-form :model="ruleForm"  :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm" label-position="top">
          ...
          <div class="condition">
            <h2>执行条件</h2>
            <p class="danger"> <i class="el-icon-warning" style="color: #3366FF"></i>  当订单状态变更为“交易已成功”触发订单催付！短信将发送至收货人手机</p>
            <el-form-item label="下单后催付提醒间隔时间" prop="hours">
              <el-input v-model="ruleForm.hours"  ></el-input>  小时
              <el-input v-model="ruleForm.minutes"  ></el-input>  分钟
            </el-form-item>
            <el-form-item label="订单金额" :prop="ruleForm.radio3 ==1 ?'money1' : 'radio3'">
          ...
          <div class="mb">
            ...
            <el-form-item label="短信内容" prop="textarea" :show-message='suc2'>
          ...
          <el-form-item class="sub">
            <el-button @click="submitForm('ruleForm',1,0)" >保存</el-button>
            <el-button type="primary" @click="submitForm('ruleForm',1,1)" >保存并启用</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
    ...
    <div class="look" v-if="look">    
      <div class="head clear">  
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item>客户关怀</el-breadcrumb-item>
          <el-breadcrumb-item>订单催付</el-breadcrumb-item>  
          <el-breadcrumb-item>订单催付详情</el-breadcrumb-item>      
        </el-breadcrumb> 
        <h1> <i class="el-icon-back" @click="back(3)"></i>  <span>订单催付详情</span>    </h1>
      ...
      <div class="box">
        ...
        <div class="condition">
          <h2>执行条件</h2>  
          <div ><span>下单后催付提醒间隔时间</span> {{ruleForm.hours}}小时{{ruleForm.minutes}}分</div>
          
          <div v-if='!ruleForm.maxPrice && !ruleForm.minPrice'><span>订单金额</span> 任意</div>
          <div v-if='!ruleForm.maxPrice && ruleForm.minPrice'><span>订单金额</span> 大于 {{ruleForm.minPrice}} 元</div>
          <div v-if='ruleForm.maxPrice && !ruleForm.minPrice'><span>订单金额</span> 小于 {{ruleForm.maxPrice}} 元</div>
          <div v-if='ruleForm.maxPrice && ruleForm.minPrice'><span>订单金额</span> {{ruleForm.minPrice}} 元 至 {{ruleForm.maxPrice}} 元 </div>
          <div>
       ...
       <div class="city">
            <div class="cityBox clear">
                <div style="border-right:0;"> 
                    <el-input class="inline-input" placeholder='省份名称' prefix-icon='el-icon-search' @keyup.native='inputChange' v-model="selCityStr"></el-input>
                    <el-checkbox-group v-model='checkAll' @change="handleCheckAllChangeAll" class="checkGroup" style="border-right:0" >
                        <template v-for='item in ruleForm.pros' >   
                            <el-checkbox :key="item.code" :class="item.sign? 'is-checked' :''" :indeterminate="item.isIndeterminate" :label='item.name'>{{item.name}}<i class="el-icon-arrow-right" ></i></el-checkbox>
                        </template>
                    </el-checkbox-group>
                </div>
        ...
        hours:[
          {
            ...
            validator:(rule, value, callback) => {
              ...
              setTimeout(() => {
                ...
                // if ((this.ruleForm.minutes*1)>999999 ||(this.ruleForm.minutes*1)<0 || (this.ruleForm.hours*1)>999999 || (this.ruleForm.hours*1)<0) {
                //   callback(new Error('请输入正确的格式'))
                // } else {
                //   callback()
                // }
      ...
      this.$http.post(''+ process.env.API_ROOT + (this.ruleForm.marketingPlanId ? '/customer/care/config/edit' : '/customer/care/config/add'),
          {
            ...
            'waitHour':this.ruleForm.hours,
            'waitMinute':this.ruleForm.minutes,
          }
        ).then(res => {
            console.log("新增订单催付", res);
                this.$message({
