<template>
  <div class="addGroup">
      ...
      <el-form
        ...
        <div class="choose">
          ...
          <el-form-item label="分组名称：" prop='name' :rules='rules.name' label-position="top">
              <el-input v-model="baseForm.name" class="name" maxlength='15' ></el-input>
          ...
          <el-form-item label="分组目录：" prop='catalog' :rules='rules.catalog' label-position="top">
            <el-select v-model="baseForm.catalog" placeholder="请选择分组目录"  class="catalog" >
              <el-option
        ...
        <div class="tag">
          ...
          <div class="base">
            ...
            <el-form-item label="生日：" :prop='form.birthCondition==1 || form.birthCondition==3 ? "date" :"daterange"' :rules='form.birthCondition==1 || form.birthCondition==3 ? rules.date :rules.daterange' label-position="left" v-if="change.value2">
              <el-select v-model="form.birthCondition"  placeholder="请选则筛选条件">
                <el-option
              ...
              <el-date-picker
                v-show="form.birthCondition==1||form.birthCondition==3"
                v-model="form.date"
                type="date"
            ...
            <el-form-item label="入会时间："  label-position="left " v-if="change.value7" :prop='form.entryTimeCondition==2? "entryTimerange":"entryTime"' :rules='form.entryTimeCondition==2 ? rules.entryTimerange:rules.entryTime'>
              ...
              <el-date-picker
                v-show="form.entryTimeCondition==1||form.entryTimeCondition==3"
              ...
              <el-date-picker
                v-show="form.entryTimeCondition==2"
                align='center'
            ...
            <el-form-item label="客户来源：" prop='source' label-position="left" v-if="change.value9" :rules='rules.source'>
              ...
              <el-select v-model="form.source" class placeholder="请选择客户来源" v-if="form.sourceCondition">
    ...
    <div class="sub" >
      ...
      <el-button size='medium' type='primary' @click="submitForm('form',2,'baseForm')">保存分组</el-button>
    ...
    <el-dialog
      ...
      <el-form label-position="top" label-width="80px" :model="catalogForm" ref="catalogForm" >
        ...
        <el-form-item label="目录名称" prop='catalogName'  :rules='rules.catalogName'>
          <el-input v-model="catalogForm.catalogName" maxlength='10' class="proName"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="qx" :plain='true'>取消</el-button>
        <el-button type="primary" @click="submitForm('catalogForm',3)">提交</el-button>
      </span>
    </el-dialog>
...
export default {
        ...
        name: [
          { required: true ,  trigger: 'blur' ,validator: (rule, value, callback) => {
            ...
            if (!value ) {
                ...
                if(!reg.test(value)){
                  callback(new Error('分组名称不能包含特殊字符'))
                }else{
                  callback()
                }
              }
  ...
  computed:{
    ...
    changeStr(){
      ...
      for (var i in this.marketing ){
        arr.push(this.marketing[i])
      }
      for (var i in this.rfm ){
        arr.push(this.rfm[i])
      }
      for (var i in this.deal ){
        arr.push(this.deal[i])
      }
      for (var i in this.pro ){
        arr.push(this.pro[i].value)
      }
      return arr.join(',')
    }
  },
    ...
    pro:{
      handler:function(n,o){
        for ( var i =0;i<n.length;i++ ){
          if(!n[i].value){
          console.log(2)
          console.log(this.form.proValue[i])
            if(this.form.proValue[i]){
          console.log(1)

              this.form.proValue[i].type=this.proList[i].customPropertyType
              this.form.proValue[i].name=this.proList[i].customPropertyName
              this.form.proValue[i].checkAll= false
              this.form.proValue[i].checkedValue= []
              this.form.proValue[i].isIndeterminate= false
              this.form.proValue[i].select =''
              this.form.proValue[i].value=''
              this.form.proValue[i].daterange=[]
              this.form.proValue[i].radio=''
            }
          }
        }
      },
  ...
  methods: {
    ...
    save(){
      ...
      var data ={
        "customerGroupName":this.baseForm.name,//分组名称
        "catalogId":this.baseForm.catalog,//目录id
            ...
            // this.id='';
              ...
              if(data[0]){
                ...
                if(data[0].basicMinPoint && data[0].basicMaxPoint){
                  ...
                  this.form.point = data[0].basicMaxPoint

                  this.form.maxPoint = ''
                  this.form.minPoint = ''
                }else if(data[0].basicMinPoint && !data[0].basicMaxPoint){
                  this.form.pointCondition = 3
                  this.form.point = data[0].basicMinPoint
                  this.form.minPoint = ''
                  this.form.maxPoint = ''
                }else{
                  this.form.pointCondition = ''
              ...
              if(data[3]){
                ...
                if(data[3].tradeSource){
                  this.form.sources = data[3].tradeSource.split(',')
                }else{
                  this.form.sources=[]
                }
                if(data[3].tradeSkuCode){
                ...
                if(obj.custom.length == 0){
                  ...
                  obj.custom.forEach((item,index) => {
                    ...
                    if(item.customLabelType == 1){
                      this.form.proValue[index].radio = item.customLabelValue
                    }
                    //复选
                    if(item.customLabelType == 2){
                      
                      this.form.proValue[index].checkedValue = item.customLabelValue.split(',')

                    }
                    //下拉
                    if(item.customLabelType == 3){
                      this.form.proValue[index].select = item.customLabelValue
                    }
                    //时间
                    if(item.customLabelType == 6){
                      this.form.proValue[index].daterange = item.customLabelValue.split(',')
                    }
                    //文本
                    if(item.customLabelType == 5){
                      this.form.proValue[index].value = item.customLabelValue
                    }
                  });
                }
              }
    ...
    //引入已有分组
    add(){
      this.$confirm('引入分组将会清空标签,确定要引入吗？','确认引入分组', {
          distinguishCancelAndClose: true,
          confirmButtonText: '确定',
          cancelButtonText: '取消'
        }).then(() => {
    ...
    submitForm(formName,type,formName1) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if(type==1){
            this.cal()
          }else if(type==2){
            if(this.$refs[formName1]){
        ...
        .then(res => {
          ...
          if (res.data.code == 1) {
            if(res.data.data.list.length!=0){
              this.baseForm.catalog = res.data.data.list[0].catalog_id
            }else{
              this.baseForm.catalog=''
            }
